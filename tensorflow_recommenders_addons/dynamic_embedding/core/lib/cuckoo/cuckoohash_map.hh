...
// todo 并发哈希table值
//operator new（1）只分配所要求的空间，不调用相关对象的构造函数
...
class cuckoohash_map {
 ...
 private:
  ...
  static size_type reserve_calc(const size_type n) {
    //这个地方的处理逻辑，我个人理解是害怕n无法整除slot_per_bucket，所以加上slot_per_bucket-1，保证整除的整数一定可以包括n
  ...
  // necessary.
  // todo 存储的容器
  // buckets_t = libcuckoo_bucket_container<Key, T, Allocator, partial_t, SLOT_PER_BUCKET>;
